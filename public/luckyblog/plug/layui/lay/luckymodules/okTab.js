...
layui.define(["element", "jquery"], function (exports) {
  ...
  okTab.prototype.navBar = function (strData) {
   ...
   //console.log(data);
    $.each(data,function (i, value){
      temp = "";
      if (value.isCheck) {
        ulItem += '<li class="layui-nav-item layui-this show_tips">';
      } else if (value.spread=="true") {
        ulItem += '<li class="layui-nav-item layui-nav-itemed show_tips">';
      } else {
        ulItem += '<li class="layui-nav-item show_tips">';
      }
      createMenu(value, (i + 1).toString());
      ulItem += temp;
      ulItem += '</li>';
    });
    return ulItem;
    ...
    if (data.children != undefined && data.children.length > 0) {
      ...
      if (data.icon != undefined && data.icon != '') {
        /*if (data.fontFamily && data.fontFamily.indexOf("layui-icon") < 0) {
          ...
          temp += ('<i class="{0} {1}" data-icon="{1}"></i>').format(data.fontFamily, data.icon);
       /* }*/
      }
  ...
  okTab.prototype.render = function (fun) {
    ...
    $.ajax({
      ...
      {
        if (data.code==1) {
          $("#navBar").html(_this.navBar(data.data));
          element.render('nav');
